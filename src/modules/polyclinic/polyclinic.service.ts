...
export class PolyClinicService {
  ...
  async getDetail(res: Res, id: number, user?: UserJwtPayload) {
    ...
    const totalRegistrant = _.size(
      _.filter(queues, (item) => item.status !== ("EMPTY" as QueueStatus)),
    );
    let userQueue = user
      ? queues.find(({ userId }) => userId == user.id)
      : null;
    const result = { ...data, queues, totalRegistrant, userQueue };
